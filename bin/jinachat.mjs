...
let followFrom = null;
let followUntil = 0;
let botStreak = 0; // 🔁 사람 개입 없이 봇끼리 연속 교환한 턴 수 — 하드 캡으로 무한루프 최후 방어
const FOLLOWUP_MS = 90_000;
const BOT_STREAK_CAP = 8; // 봇끼리 8턴 넘게 주고받으면 사람이 올 때까지 강제 침묵
const recent = [];
...
- 🤫 끝낼 때를 스스로 판단하세요. 이미 합의에 이르렀거나, 실질적으로 새로 보탤 내용이 없거나, 상대가 단순 동의·마무리·인사만 했다면 — 억지로 답하지 말고 정확히 "[침묵]" 한 단어만 출력하세요. ("저는 침묵하겠습니다" 같은 말도 하지 마세요 — 그것도 발화라 상대가 또 답하게 됩니다.) 사람이 대화하듯, 할 말 없으면 안 하는 게 자연스럽습니다.
...
function askBrain(from) {
  ...
  const c = execFile(bin, args, { cwd, timeout: 300_000, maxBuffer: 16 * 1024 * 1024 }, (e, stdout) => { // 리뷰는 몇 분 걸린다
    ...
    if (/^\[침묵\]/.test(reply.trim())) { console.log(`[${new Date().toISOString()}] 🤫 침묵 선택 — 발화 생략`); busy = false; return; } // 두뇌가 '끝낼 때'로 판단
...
if (cmd === 'bridge') {
  s.on('chat:msg', (m) => {
    ...
    // 🛡️ --only 화이트리스트 — 지정 닉 외의 발화는 절대 트리거하지 않는다 (클로 보안 리뷰 2026-07-23)
    if (only.length && !only.includes(m.nickname)) return;
    // 봇끼리 대화는 허용(교차리뷰 킬러 유즈케이스) — 자연스러운 종료는 두뇌의 [침묵] 판단이 담당. (지기 2026-07-23: 사람처럼 끝내라)
    const fromBot = Boolean(m.fromAgent);
    if (!fromBot) botStreak = 0; // 사람이 말하면 봇 연속 카운터 리셋
    ...
    if (busy) return;
    if (Date.now() - lastReplyAt < 5_000 && m.nickname !== followFrom) return; // 폭주 속도만 제한 (대화는 허용)
    if (fromBot && botStreak >= BOT_STREAK_CAP) return; // 🔁 하드 캡 — 사람 개입 전까지 봇끼리 무한교환 방어
    if (fromBot) botStreak += 1;
    busy = true;
    followFrom = m.nickname;
    askBrain(m.nickname);
  });
